"""
Data Generation Pipeline for Grid-Based Shape Recognition
Generates synthetic training data with various shapes on grids of size 1x1 to 30x30+
"""
...
class ShapeGenerator:
    """Generates synthetic training data for shape recognition."""
    ...
    def _generate_small_grid(self, h, w):
        """Generate small grid with simple shapes."""
        grid = np.zeros((h, w, self.num_colors))
        instance_map = np.zeros((h, w), dtype=np.int32)
        vertex_map = np.zeros((h, w), dtype=np.float32)
        edge_map = np.zeros((h, w), dtype=np.float32)
        ...
        color = np.random.randint(0, self.num_colors)
        shape_type = np.random.choice(['line', 'triangle', 'rectangle'])

        if shape_type == 'line':
            vertices = self._random_line(h, w)
        elif shape_type == 'triangle':
            vertices = self._random_triangle(h, w)
        else:
            vertices = self._random_rectangle(h, w)

        self._rasterize_shape(grid, instance_map, vertex_map, edge_map,
                             vertices, color, 1, filled=True)
    ...
    def _generate_normal_grid(self, h, w):
        """Generate normal grid with multiple shapes."""
        grid = np.zeros((h, w, self.num_colors))
        instance_map = np.zeros((h, w), dtype=np.int32)
        vertex_map = np.zeros((h, w), dtype=np.float32)
        edge_map = np.zeros((h, w), dtype=np.float32)
        ...
        for shape_id in range(1, num_shapes + 1):
            ...
            shape_type = np.random.choice([
                'line', 'triangle', 'rectangle', 'circle', 'irregular'
            ...
            if shape_type == 'line':
                ...
                vertices = self._random_circle(h, w)
            else:  # irregular
                vertices = self._random_irregular(h, w)

            self._rasterize_shape(grid, instance_map, vertex_map, edge_map,
                                 vertices, color, shape_id, filled)
    ...
    def _random_triangle(self, h, w):
        ...
        # For very small grids, use corners
        if h <= 2 or w <= 2:
            if h == 2 and w == 2:
                return [(0, 0), (0, 1), (1, 0)]
            elif h == 2:
    ...
    def _random_rectangle(self, h, w):
        ...
        # For very small grids, just use corners
        if h <= 2 or w <= 2:
    ...
    def _random_circle(self, h, w):
        ...
        # For very small grids, create a diamond/square approximation
        if h <= 4 or w <= 4:
            cy, cx = h // 2, w // 2
    ...
    def _rasterize_shape(self, grid, instance_map, vertex_map, edge_map,
                        vertices, color, instance_id, filled=True):
...
class ShapeDataset(Dataset):
    """PyTorch dataset for shape recognition."""
    ...
    def __init__(self, num_samples=1000, num_colors=8, min_size=1, max_size=30, cache_path=None):
        ...
        Args:
            ...
            num_colors: Number of discrete colors
            min_size: Minimum grid size
            max_size: Maximum grid size
            cache_path: Path to load/save dataset. If exists, loads from disk. If None, generates new data.
        """
        ...
        if cache_path and os.path.exists(cache_path):
            ...
            self.generator = ShapeGenerator(num_colors, min_size, max_size)
